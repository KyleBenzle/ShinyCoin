...
allCoinData %>% pivot_wider(., names_from = symbol, values_from = c(open, high, low, close))
...
#%>% dplyr::filter(., date >= as.date(startDate) & date <= as.date(endDate))
    ...
    denomPrice <- allCoinData %>% dplyr::filter(., symbol == 'BTC') %>% dplyr::select(., open, date)
    numPrice <- allCoinData %>% dplyr::filter(., symbol == 'ETH') %>% dplyr::select(., open, date)    
    
    # print(length(denomPrice$open))
    # print(length(allCoinData$open))
