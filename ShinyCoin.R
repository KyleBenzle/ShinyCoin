...
makeGraphRatio <- function(coinList, startDate, endDate, df){
  df <- df %>% dplyr::filter(., symbol %in% coinList ) %>% dplyr::filter(., date >= as.Date(startDate) & date <= as.Date(endDate))
  graph <- ggplot(df, aes(x = date, y = open, col=slug))+
    geom_line() +
    ggtitle("by date")
  return(graph)
...
#%>% dplyr::filter(., date >= as.date(startDate) & date <= as.date(endDate))
 ...
 #   numPrice <- allCoinData %>% dplyr::filter(., symbol == 'ETH') %>% dplyr::select(., open, date)    
  ...
  output$distPlotRatio <- renderPlot({
   # activeCoinData$date <- as.Date(activeCoinData$date, format = '%Y-%m-%d')
